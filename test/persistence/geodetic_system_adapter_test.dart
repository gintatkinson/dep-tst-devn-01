...
void main() {
  ...
  group('SqliteGeodeticSystemAdapter', () {
    ...
    test('saves and reads back a GeodeticSystem with defaults only', () async {
      ...
      await adapter.saveGeodeticSystem(kNodeId1, system);
      final loaded = await adapter.fetchGeodeticSystem(kNodeId1);
    ...
    test('saves and reads back with all fields', () async {
      ...
      await adapter.saveGeodeticSystem(kNodeId2, system);
      final loaded = await adapter.fetchGeodeticSystem(kNodeId2);
      expect(loaded, equals(system));
    });

    test('returns null for non-existent node', () async {
      final loaded = await adapter.fetchGeodeticSystem(kNodeIdNoSuch);
    ...
    test('overwrite: saving twice returns latest value', () async {
      ...
      await adapter.saveGeodeticSystem(kNodeId3, first);
      await adapter.saveGeodeticSystem(kNodeId3, second);
      final loaded = await adapter.fetchGeodeticSystem(kNodeId3);
